...
class SimpleEnvironmentFactory:
    """Simplified factory for CartPole-v1 and Pendulum-v1 environments only
    
    Provides a clean interface for creating the two supported environments
    with appropriate wrappers and configurations.
    """
    
    # Environment type mappings - Only CartPole-v1 and Pendulum-v1 supported
    ...
    def _create_base_env(self, env_name: str, **kwargs) -> gym.Env:
        """Create base environment"""
        # Only support standard gym environments now
        return gym.make(env_name, **kwargs)
    
    def _apply_dqn_wrappers(self, env: gym.Env, training: bool) -> gym.Env:
        """Apply DQN-specific wrappers"""
        # DQN doesn't need special wrappers for CartPole
        return env
    
    def _apply_ddpg_wrappers(self, env: gym.Env, training: bool) -> gym.Env:
        """Apply DDPG-specific wrappers"""
        # Import wrappers locally to avoid circular dependencies
        from .wrappers import ActionWrapper, RewardScaleWrapper
        
        # Apply wrappers for continuous environments
        env = ActionWrapper(env)  # Normalize actions to [-1, 1]
        env = RewardScaleWrapper(env, scale=0.1)  # Scale rewards
        return env
    ...
    def __repr__(self) -> str:
        ...
        return f"SimpleEnvironmentFactory(config_manager={self.config_manager})"
...
def create_env_for_algorithm(algorithm: str, 
    ...
    Returns:
        Configured environment
    """
    factory = SimpleEnvironmentFactory(config_manager)
